...
export const RADIUS = 150 // 링 반지름 (바깥 핀 글리프 공간 = 나머지 30)
...
export function arcPath(centerDeg: number, spreadDeg: number): string {
  const p1 = polar(centerDeg - spreadDeg)
  const p2 = polar(centerDeg + spreadDeg)
